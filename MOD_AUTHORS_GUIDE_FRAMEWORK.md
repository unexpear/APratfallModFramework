...
**Safety net (free your nodes anyway).** If you ship an [`ILifecycleHandler`](MOD_AUTHORS_GUIDE_VANILLA.md#a-third-way-to-run-code-ilifecyclehandler-and-onstart) node and forget to `QueueFree` it, the framework tries to force-unregister and free it on disable (scoped to your mod's load context) — so a leaked handler won't keep getting `OnUpdate` or pin your `AssemblyLoadContext`. **⚠ On build `24260516`+ this net is degraded:** `LifecycleManager` moved to a queue-command model and dropped the public `GetHandlers()` / `Unregister()`, so the framework can no longer enumerate and force-sweep a leaked handler — **you must `QueueFree` it yourself in `OnUnload`** (the framework still `UnpatchAll`s your Harmony patches and unloads the ALC, but can't reach into the lifecycle registry). Even where the sweep works, it only covers `ILifecycleHandler` nodes loaded from your assembly, not other persistence (your own `Harmony` instance, static `+=` subscriptions, timers/threads). It's a backstop for an honest mistake, not a sandbox.
...
- **One array is genuinely save-coupled: `PlayerColorsConfig.Colors`.** `Savegame` stores five plain-int indices into it — `PlayerColorIndex` / `NoseColorIndex` / `ShirtColorIndex` / `PantsColorIndex` / `ShoeColorIndex` (the last two added in build 23570525) — so inserting/reordering colors corrupts existing players' saved cosmetics. **`GameModeManager.Modes` and `LevelManager.LevelPrefabs` are NOT saved by index — adding a game mode or level is safe** (verified against `Savegame`; dev-confirmed the game mode isn't persisted by index). `ProceduralCaveComponent.BiomeGenerationConfigs` isn't a save issue but is multiplayer-determinism-sensitive — keep it identical across the lobby. See the vanilla guide's [Save-coupled arrays](MOD_AUTHORS_GUIDE_VANILLA.md#save-coupled-arrays--dont-mutate).
