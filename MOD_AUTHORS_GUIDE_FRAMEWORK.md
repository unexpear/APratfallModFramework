...
- **One array is genuinely save-coupled: `PlayerColorsConfig.Colors`.** `Savegame` stores `PlayerColorIndex` / `NoseColorIndex` / `ShirtColorIndex` as plain ints into it, so inserting/reordering colors corrupts existing players' saved cosmetics. **`GameModeManager.Modes` and `LevelManager.LevelPrefabs` are NOT saved by index — adding a game mode or level is safe** (verified against `Savegame`; dev-confirmed the game mode isn't persisted by index). `ProceduralCaveComponent.BiomeGenerationConfigs` isn't a save issue but is multiplayer-determinism-sensitive — keep it identical across the lobby. See the vanilla guide's [Save-coupled arrays](MOD_AUTHORS_GUIDE_VANILLA.md#save-coupled-arrays--dont-mutate).
