...
- [x] **Zip-drop install (shipped 2026-05-19)** — `ModZipDropInstaller.ExtractAll` runs once at framework init before `ScanLocalMods`. For each `.zip` in our scan roots (excluding Steam Workshop content): extract to same-named folder via .NET's `ZipFile.ExtractToDirectory` (zip-slip-safe in .NET 7+), then delete the source zip on success. Uses a temp folder + rename so a partial extraction never leaves a broken mod folder. Skips if the destination folder already exists (never overwrites silently).
- [x] **`--qh-mod-directory` honor + profile-isolated state (shipped 2026-05-19)** — Tim's 2026-05-18 fix removed the launch-args confirmation dialog that previously blocked Thunderstore/r2modman compat. The framework now detects `--qh-mod-directory <path>` from CLI args (`FrameworkProfile.ProfileModsDirectory`) and: (a) treats that path as the FIRST mod-scan root, so r2modman-installed mods appear in our Mods dialog; (b) writes/reads `modframework-state.json` to/from that path instead of `user://` so each r2modman profile has independent enabled-mod + approved-fingerprint state; (c) skips the default `<game>/mods/` scan to avoid leaking the user's standalone-install state into the profile. Steam Workshop content folder is still scanned globally (subscriptions are per-Steam-account, not per-profile). One-time migration: a fresh profile reads the default user:// state once on first launch so existing approvals don't get lost. Combined with zip-drop, completes the minimum-viable Thunderstore/r2modman compat path. Snake_case Thunderstore manifest parsing remains backlog.
...
- [ ] **Opt-in bypass for the user-check (🔍) gate** — power-user toggle to skip the per-mod fingerprint approval before loading. Default: gate ON (current behavior; mods stay unloaded until 🔍-scanned or toggle-on). When set OFF: framework loads mods without checking `_checkedFingerprints` — `LoadLocalAssemblyMods` skips its `IsModChecked` guard. **No warning prompt on toggle** (the user explicitly opted in). Persists across launches via the framework's own config so it survives restarts. Surface in the Mods dialog footer or a Settings entry. Use case: people who manage their own mod set and find the per-mod approval friction; risk of mod-update-introduced malicious bytes is on them. (~half day)
...
- [x] **Host-config-pushed-to-clients (CSync pattern)** — `ConfigDescription.Synced = true` opts an entry in; framework auto-sends host's value on join (full snapshot across all loaded mods) and on change (single-entry delta). Peers apply via `SetFromHost` which bypasses local save + broadcast hook (no cycle). Supports bool/int/long/float/double/string/enum via `(TypeName, StringValue)` discriminator. 32700-byte snapshot cap. Self-test step 16 covers EnumerateSyncedEntries filter, JSON wire round-trip, SetFromHost no-cycle invariant, host-side broadcast hook. (~2 days estimate → shipped)
