  ...
  ""multiplayer"": {{ ""mode"": ""local_only"", ""requires"": [], ""conflictsWith"": [] }}
    ...
    private void DoUninstall()
    {
        ...
        if (restoreDecision == RestoreDecision.Refuse)
        {
            ...
            // aborting leaves the working patched install intact so nothing breaks.
            Log($"Error: {restoreMessage}");
            throw new InvalidOperationException(restoreMessage);
        }
        if (restoreDecision == RestoreDecision.Restore)
        {
